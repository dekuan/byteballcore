	...
	(
		...
		function( last_stable_mci )
		{
			...
			( max_spendable_mc_index > 0 )
				? buildPaidWitnessesTillMainChainIndex( conn, max_spendable_mc_index, cb )
				: cb();
		}
	);
}
...
function buildPaidWitnessesTillMainChainIndex( conn, to_main_chain_index, cb )
{
	...
	conn.query
	(
		"SELECT MIN(main_chain_index) AS min_main_chain_index " +
		"FROM balls " + cross + " JOIN units USING(unit) " +
		...
		function( rows )
		{
			...
			//	...
			buildPaidWitnessesForMainChainIndex
			(
				conn,
				main_chain_index,
				onIndexDone
	...
	(
		...
		function( rows )
		{
			...
			if ( count_on_stable_mc !== count )
			{
				throw Error
				(
					"not enough stable MC units yet after MC index " +
					main_chain_index +
					": count_on_stable_mc=" +
					count_on_stable_mc +
					", count=" + count
				...
				function( arrWitnesses )
				{
					...
					(
						...
						function()
						{
							conn.query
							(
								"SELECT * FROM units WHERE main_chain_index=?",
								[
									main_chain_index
								],
								function( rows )
...
function buildPaidWitnesses( conn, objUnitProps, arrWitnesses, onDone )
{
	function updateCountPaidWitnesses( count_paid_witnesses )
	{
		conn.query
		(
			"UPDATE balls SET count_paid_witnesses=? WHERE unit=?",
			[
	...
	(
		...
		function( arrUnits )
		{
			rt	+= Date.now() - t;
			t	= Date.now();
			var strUnitsList	= ( arrUnits.length === 0 )
				? 'NULL'
				: arrUnits.map
				(
					function( unit )
			...
			(
				//
				//	we don't care if the unit is majority witnessed by the unit-designated witnesses
