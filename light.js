				...
				function(err, arrUnstableMcJoints, arrWitnessChangeAndDefinitionJoints, last_ball_unit, last_ball_mci){
					...
					async.eachSeries(
						...
						function(){
							...
							log.consoleLog("prepareHistory for addresses "+(arrAddresses || []).join(', ')+" and joints "+(arrRequestedJoints || []).join(', ')+" took "+(Date.now()-start_ts)+'ms');
							unlock();
						}
					);
				}
...
function fixIsSpentFlag(onDone){
	db.query(
		...
		function(rows){
			log.consoleLog(rows.length+" previous outputs appear to be spent");
			if (rows.length === 0)
				return onDone();
			var arrQueries = [];
			rows.forEach(function(row){
				log.consoleLog('fixing is_spent for output', row);
...
function fixInputAddress(onDone){
	db.query(
		...
		function(rows){
			log.consoleLog(rows.length+" previous inputs appear to be without address");
			if (rows.length === 0)
				return onDone();
			var arrQueries = [];
			rows.forEach(function(row){
				log.consoleLog('fixing input address for output', row);
				db.addQuery(arrQueries, 
...
function prepareLinkProofs(arrUnits, callbacks){
	...
	mutex.lock(['prepareLinkProofs'], function(unlock){
		...
		async.forEachOfSeries(
			...
			function(err){
				log.consoleLog("prepareLinkProofs for units "+arrUnits.join(', ')+" took "+(Date.now()-start_ts)+'ms, err='+err);
