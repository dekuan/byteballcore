...
function validateDefinition( conn, arrDefinition, objUnit, objValidationState, arrAuthentifierPaths, bAssetCondition, handleResult )
{
	function getFilterError( filter )
	{
		...
		if ( "asset" in filter &&
			! (
				filter.asset === "base" ||
				isStringOfLength( filter.asset, constants.HASH_LENGTH ) ||
				bAssetCondition && filter.asset === "this asset"
			) )
		{
		...
		if ( "own_funds" in filter &&
			typeof filter.own_funds !== "boolean" )
		{
		...
		if ( "address" in filter &&
			! isValidAddress( filter.address ) && filter.address !== 'this address' )
		{
			//	it is ok if the address was never used yet
	...
	function determineIfAnyOfAssetsIsPrivate( arrAssets, cb )
	{
		...
		conn.query
		(
			"SELECT 1 FROM assets WHERE unit IN(?) AND is_private=1 LIMIT 1",
			[
				arrAssets
			],
			function( rows )
	...
	function needToEvaluateNestedAddress( path )
	{
		...
		{
			//	no signatures, just validating a new definition
			return true;
		}
		if ( objValidationState.last_ball_mci < 1400000 )
		{
			//	skipping is enabled after this mci
			return true;
		}
	...
	function evaluate( arr, path, bInNegation, cb )
	{
		...
		switch( op )
		{
				...
				(
					...
					function( err )
					{
						...
						(
							null,
							(
								op === "and" && count_options_with_sig > 0 ||
								op === "or" && count_options_with_sig === args.length
							)
						);
					}
				);
			...
			case 'r of set':
				...
				(
					...
					function( err )
					{
						...
						(
							null,
							args.required > count_options_without_sig
						);
					}
				);
				break;
			...
			case 'weighted and':
				...
				(
					...
					function( err )
					{
						...
						(
							null,
							args.required > weight_of_options_without_sig
						);
					}
				);
				break;
			...
			case 'address':
				...
				(
					...
					objValidationState.last_ball_mci,
					{
						...
						{
							log.consoleLog( "inner address:", arrInnerAddressDefinition );
							needToEvaluateNestedAddress( path )
								? evaluate( arrInnerAddressDefinition, path, bInNegation, cb )
								: cb( null, true );
						},
						ifDefinitionNotFound : function( definition_chash )
						{
							...
							(
								...
								{
									return (
										author.address === other_address &&
										author.definition &&
										objectHash.getChash160( author.definition ) === definition_chash
									);
								}
							);
							if ( arrDefiningAuthors.length === 0 )
							{
								//	no address definition in the current unit
								return bAllowUnresolvedInnerDefinitions
									? cb( null, true )
									: cb( "definition of inner address " + other_address + " not found" );
							}
				...
				(
					...
					function( rows )
					{
						...
						try
						{
							...
							log.consoleLog( require('util').inspect( arrFilledTemplate, { depth: null } ) );
						}
						catch( e )
						{
			...
			case 'has definition change':
				...
				if ( bAssetCondition &&
					( changed_address === 'this address' || new_definition_chash === 'this address' ) )
				{
			...
			case 'in data feed':
				...
				// 1 complexity point for each address (1 point was already counted)
			...
			case 'seen':
				...
				//	...
				determineIfAnyOfAssetsIsPrivate
				(
					[ args.asset ],
					function( bPrivate )
					{
			...
			case 'has one equal':
				...
				if ( args.equal_fields.indexOf( "type" ) >= 0 &&
					( args.search_criteria[ 0 ].what === "output" || args.search_criteria[1].what === "output" ) )
				{
		...
		switch( op )
		{
					...
					function( arg, cb3 )
					{
						...
						(
							...
							function( arg_res )
							{
								...
								// check all members, even if required minimum already found
								cb3();
								//res ? cb3("found") : cb3();
							}
						);
					...
					function( arg, cb3 )
					{
						...
						(
							...
							function( arg_res )
							{
								...
								//	check all members, even if required minimum already found, so that we don't allow invalid sig on unchecked path
								cb3();
			...
			case 'sig':
				...
				if ( algo === 'secp256k1' )
				{
					...
					//	...
					var res = ecdsaSig.verify
					(
						objValidationState.unit_hash_to_sign,
						signature,
						args.pubkey
					);
					if ( ! res )
				...
				(
					...
					objValidationState.last_ball_mci,
					{
						...
						ifDefinitionNotFound : function( definition_chash )
						{
							...
							if ( arrDefiningAuthors.length === 0 )
							{
								//	no definition in the current unit
								return cb2( false );
							}
							if ( arrDefiningAuthors.length > 1 )
							{
			...
			case 'cosigned by':
				...
				log.consoleLog( op + " " + arrAuthorAddresses.indexOf( cosigner_address ) );

				//	...
				cb2
				(
					arrAuthorAddresses.indexOf( cosigner_address ) >= 0
				);
				break;
