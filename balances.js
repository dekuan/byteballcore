...
function readBalance( wallet, handleBalance )
{
	...
	//	...
	assocBalances[ constants.BLACKBYTES_ASSET ] =
		{
			is_private	: 1,
			stable		: 0,
			pending		: 0
...
function readSharedAddressesOnWallet( wallet, handleSharedAddresses )
{
	db.query
	(
		"SELECT DISTINCT shared_address " +
		"FROM my_addresses JOIN shared_address_signing_paths USING(address) " +
		"WHERE wallet=?",
		[
			wallet
		],
		function( rows )
		{
			var arrSharedAddresses = rows.map
			(
				function( row )
				{
...
function readSharedAddressesDependingOnAddresses( arrMemberAddresses, handleSharedAddresses )
{
	...
	(
		...
		"WHERE address IN(" + strAddressList + ")",
		function( rows )
		{
			var arrSharedAddresses	= rows.map
			(
...
function readSharedBalance( wallet, handleBalance )
{
	...
	//	...
	readSharedAddressesOnWallet
	(
		wallet,
		function( arrSharedAddresses )
		{
			if ( arrSharedAddresses.length === 0 )
