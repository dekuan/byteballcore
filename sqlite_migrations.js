...
{
	connection.db[ bCordova ? 'query' : 'all' ]
	(
		"PRAGMA user_version",
		function( err, result )
		{
			...
			log.consoleLog( "db version " + version + ", software version " + VERSION );
				...
				[
					function( cb )
					{
						...
						if ( version < 16 )
						{
							...
							(
								arrQueries,
								"CREATE TABLE IF NOT EXISTS sent_mnemonics ( \n\
						...
						if ( version < 17 )
						{
							...
							(
								arrQueries,
								"CREATE TABLE IF NOT EXISTS private_profile_fields ( \n\
					...
					function( cb )
					{
						if ( version < 18 )
						{
							...
							(
								arrQueries,
								"CREATE TABLE IF NOT EXISTS attested_fields ( \n\
								unit CHAR(44) NOT NULL, \n\
							...
							connection.addQuery( arrQueries, "CREATE INDEX IF NOT EXISTS attestedFieldsByAttestorFieldValue ON attested_fields(attestor_address, `field`, `value`)" );
							connection.addQuery( arrQueries, "CREATE INDEX IF NOT EXISTS attestedFieldsByAddressField ON attested_fields(address, `field`)" );
							connection.addQuery
							(
								arrQueries,
								"CREATE TABLE IF NOT EXISTS original_addresses ( \n\
								unit CHAR(44) NOT NULL, \n\
								address CHAR(32) NOT NULL, \n\
