...
function loadBitcoreFromNearestParent(mod){
	...
	try{
		...
		log.consoleLog("bitcore-lib not found from "+mod.filename+", will try from its parent");
...
function checkAndFinalizeWallet(wallet, onDone){
	db.query("SELECT member_ready_date FROM wallets LEFT JOIN extended_pubkeys USING(wallet) WHERE wallets.wallet=?", [wallet], function(rows){
		...
		//	throw Error("no wallet in checkAndFinalizeWallet");
			log.consoleLog("no wallet in checkAndFinalizeWallet");
...
function addWallet(wallet, xPubKey, account, arrWalletDefinitionTemplate, onDone){
	...
	async.series([
		...
		function(cb){
			async.eachSeries(
				...
				function(device_address, cb2){
					log.consoleLog("adding device "+device_address+' to wallet '+wallet);
		...
		function(cb){
			...
			async.eachSeries(
				...
				function(signing_path, cb2){
					log.consoleLog("adding signing path "+signing_path+' to wallet '+wallet);
		...
		log.consoleLog("addWallet done "+wallet);
...
function createWallet(xPubKey, account, arrWalletDefinitionTemplate, walletName, isSingleAddress, handleWallet){
	...
	log.consoleLog('will create wallet '+wallet);
	...
	addWallet(wallet, xPubKey, account, arrWalletDefinitionTemplate, function(){
		...
		if (arrDeviceAddresses.length === 1) // single sig
			return;
		log.consoleLog("will send offers");
		...
		device.readCorrespondentsByDeviceAddresses(arrDeviceAddresses, function(arrOtherCosigners){
			...
			arrDeviceAddresses.forEach(function(device_address){
				if (device_address === device.getMyDeviceAddress())
					return;
				log.consoleLog("sending offer to "+device_address);
...
function createWalletByDevices(xPubKey, account, count_required_signatures, arrOtherDeviceAddresses, walletName, isSingleAddress, handleWallet){
	log.consoleLog('createWalletByDevices: xPubKey='+xPubKey+", account="+account);
...
function cancelWallet(wallet, arrDeviceAddresses, arrOtherCosigners){
	log.consoleLog("canceling wallet "+wallet);
		...
		db.query(
			...
			function(rows){
				...
				rows.forEach(function(row){
					...
					log.consoleLog('pubkey for wallet '+wallet+' path '+path+' device '+row.device_address+' xpub '+row.extended_pubkey+': '+params['pubkey@'+row.device_address]);
				});
...
function forwardPrivateChainsToOtherMembersOfWallets(arrChains, arrWallets, conn, onSaved){
	log.consoleLog("forwardPrivateChainsToOtherMembersOfWallets", arrWallets);
	conn = conn || db;
	conn.query(
...
function forwardPrivateChainsToOtherMembersOfAddresses(arrChains, arrAddresses, conn, onSaved){
	log.consoleLog("forwardPrivateChainsToOtherMembersOfAddresses", arrAddresses);
	conn = conn || db;
