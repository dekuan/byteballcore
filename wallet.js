...
function handleJustsaying( ws, subject, body )
{
	switch( subject )
	{
		...
		case 'hub/message':
			...
			db.query
			(
				"SELECT hub, is_indirect FROM correspondent_devices WHERE device_address=?",
				[
					from_address
				],
				function( rows )
				{
					if ( rows.length > 0 )
					{
						if ( json.device_hub && json.device_hub !== rows[ 0 ].hub )
						{
							//	update correspondent's home address if necessary
							db.query
							(
								"UPDATE correspondent_devices SET hub=? WHERE device_address=?",
								[
									json.device_hub,
									from_address
								],
...
function handleMessageFromHub( ws, json, device_pubkey, bIndirectCorrespondent, callbacks )
{
	...
	switch( subject )
	{
		...
		case "create_new_shared_address" :
			...
			(
				body.address_definition_template,
				from_address,
				function( err, assocMemberDeviceAddressesBySigningPaths )
				{
					...
					//
					//	this event should trigger a confirmatin dialog,
					//	user needs to approve creation of the shared address and choose his
					...
					(
						...
						body.address_definition_template,
						assocMemberDeviceAddressesBySigningPaths
					);
					callbacks.ifOk();
				}
			);
		...
		case "sign" :
			...
			if ( "private_payloads" in body )
			{
				...
				for ( var payload_hash in assocPrivatePayloads )
				{
					...
					{
						//	indivisible asset.
						//	In this case, payload hash is calculated based on output_hash rather than address and blinding
			...
			(
				...
				body.signing_path,
				{
					...
					ifLocal : function( objAddress )
					{
						...
						//	as soon as the previously started validation finishes,
						//		it will trigger our event handler (as well as its own)
					...
					ifRemote : function( device_address )
					{
						...
						eventBus.once
						(
							"signature-" + device_address + "-" + body.address + "-" + body.signing_path + "-" + text_to_sign,
							function( sig )
							{
...
function readAssetMetadata( arrAssets, handleMetadata )
{
	...
	(
		...
		function( rows )
		{
			...
			var updateAssets = conf.bLight
				?
				network.requestProofsOfJointsIfNewOrUnstable
				: function(arrAssets, onDone){ onDone(); };
						...
						metadata_unit,
						{
							...
							ifFound : function( objJoint )
							{
								objJoint.unit.messages.forEach( function( message )
								{
									...
									(
										"INSERT " + db.getIgnore()
										+ " INTO asset_metadata (asset, metadata_unit, registry_address, suffix, name, decimals) \n\
										VALUES (?,?,?, ?,?,?)",
										[
											asset,
											metadata_unit,
		...
		function( rows )
		{
			...
			(
				...
				function( objAsset )
				{
					...
					{
						// don't shorten the list of addresses, indivisible_asset.js will do it later according to denominations
						estimated_amount = 0;
					}

					if ( ! objAsset.cap )
					{
						// uncapped asset: can be issued from definer_address or from any address
						var and_address = objAsset.issued_by_definer_only ? " AND address="+db.escape(objAsset.definer_address) : '';

						db.query
						(
							"SELECT address FROM my_addresses WHERE wallet=? " + and_address + " LIMIT 1",
							[
								wallet
							],
							function( rows )
					...
					handleFundedAddresses
					(
						composer.filterMostFundedAddresses( rows, estimated_amount )
					);
				}
			);
...
function readAdditionalSigningAddresses( arrPayingAddresses, arrSigningAddresses, arrSigningDeviceAddresses, handleAdditionalSigningAddresses )
{
	...
	(
		...
		function( rows )
		{
			...
			(
				[],
				arrSigningAddresses.concat( arrAdditionalAddresses ),
				arrSigningDeviceAddresses,
				function( arrMoreAddresses )
				{
		...
		function( arrFundedAddresses )
		{
			...
			var addSigningAddressesAndReturn = function()
			{
				...
				(
					...
					arrSigningAddresses,
					arrSigningDeviceAddresses,
					function( arrAdditionalAddresses )
					{
						handleFundedAndSigningAddresses
						(
							arrFundedAddresses,
			...
			(
				...
				wallet,
				TYPICAL_FEE,
				function( _arrBaseFundedAddresses )
				{
	...
	return {
		readSigningPaths	: function( conn, address, handleLengthsBySigningPaths )
		{
			...
			(
				...
				address,
				arrSigningDeviceAddresses,
				function( assocTypesBySigningPaths )
				{
				...
				signing_path,
				{
					...
					ifLocal : function( objAddress )
					{
						...
						(
							objAddress.wallet,
							objAddress.account,
							objAddress.is_change,
							objAddress.address_index,
							buf_to_sign,
							function( sig )
							{
					...
					ifRemote : function( device_address )
					{
						...
						eventBus.once
						(
							"signature-" + device_address + "-" + address + "-" + signing_path + "-" + buf_to_sign.toString( "base64" ),
							function( sig )
							{
								handleSignature( null, sig );
								if ( sig === '[refused]' )
...
function forwardPrivateChainsToOtherMembersOfSharedAddresses(
	...
	arrPayingAddresses,
	excluded_device_address,
	bForwarded,
	conn,
	onDone )
{
	walletDefinedByAddresses.readAllControlAddresses
	(
		conn,
		arrPayingAddresses,
		function( arrControlAddresses, arrControlDeviceAddresses )
		{
			arrControlDeviceAddresses	= arrControlDeviceAddresses.filter
			(
			...
			walletDefinedByKeys.readDeviceAddressesControllingPaymentAddresses
			(
				conn,
				arrControlAddresses,
				function( arrMultisigDeviceAddresses )
				{
					...
					walletGeneral.forwardPrivateChainsToDevices
					(
						arrControlDeviceAddresses,
						arrChainsOfCosignerPrivateElements,
						bForwarded,
						conn,
						function()
...
function replaceInTextcoinTemplate( params, handleText )
{
	...
	(
		...
		function( err, template )
		{
			...
			var text = "Here is your link to receive " + params.amount
				+ " " + params.asset + params.usd_amount_str
				+ ": https://byteball.org/#textcoin?" + params.mnemonic;
	...
	var signer	=
	{
		...
		sign : function( objUnsignedUnit, assocPrivatePayloads, address, signing_path, handleSignature )
		{
			...
			(
				...
				ecdsaSig.sign
				(
					objectHash.getUnitHashToSign( objUnsignedUnit ),
					xPrivKey.privateKey.bn.toBuffer( { size : 32 } )
				)
			);
		}
	};
...
function determineIfDeviceCanBeRemoved( device_address, handleResult )
{
	...
	(
		...
		function( correspondent )
		{
			...
			//	...
			readDeviceAddressesUsedInSigningPaths
			(
				function( arrDeviceAddresses )
				{
