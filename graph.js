...
function compareUnitsByProps( conn, objUnitProps1, objUnitProps2, handleResult )
{
	...
	if ( objUnitProps1.is_free === 1 &&
		objUnitProps2.is_free === 1 )
	{
		//	free units
		return handleResult( null );
	}
	...
	if ( objUnitProps1.latest_included_mc_index >= objUnitProps2.main_chain_index &&
		objUnitProps2.main_chain_index !== null )
	{
		return handleResult( +1 );
	}
	if ( objUnitProps2.latest_included_mc_index >= objUnitProps1.main_chain_index &&
		objUnitProps1.main_chain_index !== null )
	{
		return handleResult( -1 );
	}
	...
	//	...
	( later_unit_delta > earlier_unit_delta )
		? goUp( [ objLaterUnit.unit ] )
		: goDown( [ objEarlierUnit.unit ] );
}
	...
	(
		...
		function( objEarlierUnitProps, arrLaterUnitProps )
		{
			...
			if ( objEarlierUnitProps.main_chain_index !== null &&
				max_later_limci >= objEarlierUnitProps.main_chain_index )
			{
				return handleResult( true );
			}
				...
				(
					...
					function( rows )
					{
						...
						for ( var i = 0; i < rows.length; i++ )
						{
							...
							if ( objUnitProps.is_on_main_chain === 0 &&
								objUnitProps.level > objEarlierUnitProps.level )
							{
								arrNewStartUnits.push( objUnitProps.unit );
							}
						}
		...
		(
			...
			function( rows )
			{
				...
				for ( var i = 0; i < rows.length; i++ )
				{
					...
					if ( objUnitProps.main_chain_index !== null &&
						objUnitProps.main_chain_index <= max_later_limci )
					{
						//	exclude free balls!
				...
				( arrNewStartUnits.length > 0 )
					? goDown( arrNewStartUnits )
					: handleUnits( arrLandedUnits, arrUnlandedUnits );
			}
		);
	}
...
{
	var arrLaterUnits	= arrLaterUnitProps.map
	(
		function( objLaterUnitProps )
		{
			return objLaterUnitProps.unit;
		}
	);
	var max_later_limci	= Math.max.apply
	(
	...
	//	...
	arrLaterUnitProps.forEach
	(
		function( objUnitProps )
		{
			if ( objUnitProps.latest_included_mc_index >= objEarlierUnitProps.main_chain_index )
			{
				arrLandedUnits.push( objUnitProps.unit );
