		...
		function( unlock )
		{
			...
			(
				...
				function( err )
				{
					...
					log.consoleLog
					(
						"prepareCatchupChain since mci " + last_stable_mci + " took " + ( Date.now() - start_ts ) + 'ms'
					);

					//
					//	unlock from this point
					//
					unlock();
				}
			);
					...
					function( cb )
					{
						...
						(
							...
							function( rows )
							{
								...
								storage.readLastStableMcUnitProps
								(
									db,
									function( objLastStableMcUnitProps )
									{
...
function processHashTree( arrBalls, callbacks )
{
	...
	(
		...
		function( unlock )
		{
			db.query
			(
				"SELECT 1 FROM hash_tree_balls LIMIT 1",
				function( ht_rows )
				{
								...
								function()
								{
									...
									(
										...
										function( objBall, cb )
										{
											...
											function addBall()
											{
												//	insert even if it already exists in balls,
												//	because we need to define max_mci by looking outside this hash tree
												conn.query
												(
